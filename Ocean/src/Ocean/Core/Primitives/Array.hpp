...
#include "Ocean/Core/Primitives/ADTs/List.hpp"
...
namespace Ocean {
	...
	template <typename T>
	class DynamicArray : public ADT::List<T> {
	public:
		...
		virtual b8 Equals(Object* rhs) const override;
	...
	template <typename T>
	class FixedArray : public ADT::List<T> {
	public:
		FixedArray() = default;
	...
	template<typename T>
	inline void DynamicArray<T>::Init(Allocator* allocator) {
	...
	template<typename T>
	inline u32 DynamicArray<T>::IndexOf(const T& element) const {
		for (u32 i = 0; i < m_Size; i++) {
			if (p_Data[i] == element)
				return i;
		}
	...
	template<typename T>
	inline b8 DynamicArray<T>::Equals(Object* rhs) const {
		return false;
	}
	...
	template<typename T>
	inline u32 FixedArray<T>::IndexOf(const T& element) const {
		for (u32 i = 0; i < m_Size; i++) {
			if (p_Data[i] == element)
				return i;
		}
	...
	template<typename T>
	inline b8 FixedArray<T>::Equals(Object* rhs) const {
		return false;
	}
