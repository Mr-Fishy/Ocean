...
void sonar::convert::allocatebuffer(std::string name, IPLAudioBuffer &buff){
    ...
    sonar::global_audio_context::buffers[name.c_str()] = MakeRef<IPLAudioBuffer>(buffer);
...
void sonar::convert::deinterleave(std::string name, float* indata, int channels, int samples){
    ...
    sonar::global_audio_context::buffers[name.c_str()] =temp;
...
void sonar::convert::interleave(std::string name, float* outdata){
    ...
    iplAudioBufferInterleave(*sonar::steamaudio::context, sonar::global_audio_context::buffers[name.c_str()].get(), outdata);
